...
if not st.session_state.get('logged_in'): st.warning("Por favor, faça o login."); st.stop()
user_data = st.session_state.get('user_data', {})
if user_data.get('role') != 'motorista': st.error("Acesso negado."); st.stop()

st.title(f"📋 Checklist Pré-Jornada, {user_data.get('email')}")
gestor_uid = user_data.get('gestor_uid')
if not gestor_uid: st.error("Este motorista não está associado a um gestor."); st.stop()
...
if not gestor_email_acesso or not gestor_etrac_api_key: st.error("Seu gestor não possui credenciais da eTrac configuradas."); st.stop()

vehicles = etrac_service.get_vehicles_from_etrac(gestor_email_acesso, gestor_etrac_api_key)
if not vehicles: st.warning("Nenhum veículo retornado pela API da eTrac."); st.stop()
        ...
        if st.form_submit_button("Enviar Checklist"):
            ...
            if not is_ok and not notes:
                ...
                checklist_data = {
                    ...
                    "timestamp": datetime.now(), "items": results, "notes": notes, "status": "Aprovado" if is_ok else "Pendente"
                }
